-- ====================================================
-- Netgear Financial Monitor - 修复版种子数据
-- 简化版本，移除ON CONFLICT子句
-- ====================================================
...
BEGIN
    ...
    -- 清理现有数据（如果存在）
    -- ====================================================
    DELETE FROM market_metrics WHERE period = q1_2025_period;
    DELETE FROM competitor_data WHERE period = q1_2025_period;
    DELETE FROM milestone_events WHERE company_id = netgear_id AND event_date >= '2024-01-01';
    DELETE FROM geographic_revenue WHERE company_id = netgear_id AND period = q1_2025_period;
    DELETE FROM product_line_revenue WHERE company_id = netgear_id AND period = q1_2025_period;
    ...
    RAISE NOTICE '修复版种子数据插入完成!';
    RAISE NOTICE '插入的数据包括:';
    RAISE NOTICE '- 产品线数据: 10条记录';
    RAISE NOTICE '- 地理分布: 3条记录';
    RAISE NOTICE '- 里程碑事件: 9条记录';
    RAISE NOTICE '- 竞争对手数据: 1条记录';
    RAISE NOTICE '- 市场指标: 4条记录';
